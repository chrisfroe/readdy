...
private:
    ...
    LJResult simulate(const std::vector<readdy::Vec3> &initPos, bool soft, std::size_t nSteps = 10000) {
        ...
        //readdy::log::info("rank-{} -- before add particles", kernel.domain().rank());
        ...
        //readdy::log::info("rank-{} -- before creation of other actions", kernel.domain().rank());
        ...
        //readdy::log::info("rank-{} -- before step 0", kernel.domain().rank());

        neighborList->perform();
        forces->perform();
        eval(0);
        MPI_Barrier(kernel.commUsedRanks());
        //readdy::log::info("rank-{} -- before loop", kernel.domain().rank());
        ...
        progress.done();
        //readdy::log::info("rank-{} -- after loop", kernel.domain().rank());
        if (kernel.domain().isMasterRank()) {
        ...
        //readdy::log::info("rank-{} -- after process results", kernel.domain().rank());
        result.performance = Json::parse(readdy::util::Timer::perfToJsonString());
        readdy::util::Timer::clear();
        //readdy::log::info("rank-{} -- after getting performance", kernel.domain().rank());
        //auto jsonresult = Json(result).dump(2);
        //readdy::log::info("rank-{} -- results {}", kernel.domain().rank(), jsonresult);
        return result;
    }
};
